 ...
 * Rounding drift is applied to the largest shortfall so the parts sum to amount.
...
/**
 * Validates amount against combined remaining and returns the auto-split.
 * @param buckets - Current fair-share buckets
 * @param payerId - Partner paying
 * @param receiverId - Partner receiving
 * @param amount - Transfer amount
 */
export function resolveCombinedTransfer(
  buckets: { investment: BucketNets; expenses: BucketNets; emi: BucketNets },
  payerId: string,
  receiverId: string,
  amount: number
): { ok: true; allocations: TransferAllocations; maxAmount: number } | { ok: false; error: string } {
  const payerNets = partnerBucketNets(buckets, payerId);
  const receiverNets = partnerBucketNets(buckets, receiverId);
  const maxAmount = maxPairTransfer(payerNets, receiverNets);
  if (maxAmount <= 0.01) {
    return { ok: false, error: 'No remaining fair-share balance between these partners' };
  }
  if (amount - maxAmount > 0.01) {
    return {
      ok: false,
      error: `Amount exceeds remaining ${maxAmount.toFixed(2)} between these partners`,
    };
  }
  const allocations = splitTransferAmount(amount, bucketShortfalls(payerNets));
  const allocated = roundMoney(allocations.investment + allocations.expenses + allocations.emi);
  if (allocated <= 0.01) {
    return { ok: false, error: 'Nothing left to allocate across investment, expenses, or EMI' };
  }
  return { ok: true, allocations, maxAmount };
}
